...
function Home() {
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    /* @__PURE__ */ jsx("h1", { children: "vr12        Home          no 18      15:00    29/02/2024 " }),
    /* @__PURE__ */ jsx("div", { children: addAndMultiply(1, 2, 3) }),
    /* @__PURE__ */ jsx("div", { children: multiplyAndAdd(1, 2, 3) })
  ] });
}
...
function ToProxy() {
  ...
  async function getData() {
    await axios.get(urlProxy).then((res) => setData(res.data)).catch((error) => {
      console.log("urlProxy = ,catch error = ", urlProxy, error);
    });
    await axios.get(urlProxy2).then((res) => setData2(res.data)).catch((error2) => {
      console.log("urlProxy2 = ,catch error2 = ", urlProxy2, error2);
    });
    await axios.get(urlProxy3).then((res) => setData3(res.data)).catch((error3) => {
